This directory contains the ReadercoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Readercoin UI.
...
ClientModel is used by the main application `readercoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/readercoin/readercoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a readercoin:... link from a browser).
...
* UI elements like ReadercoinAmountField, which inherit from QWidget.
* `readercoinstrings.cpp`: automatically generated
* `readercoinunits.(h/cpp)`: RDC / mRDC / etc handling
* `callback.h`
...
* don't change `local/readercoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "readercoin-qt" under "Run", which is where you can also add command line arguments)
