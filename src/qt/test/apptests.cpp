...
//! Entry point for ReadercoinApplication tests.
void AppTests::appTests()
{
    ...
    if (QApplication::platformName() == "minimal") {
        ...
        QWARN("Skipping AppTests on mac build with 'minimal' platform set due to Qt bugs. To run AppTests, invoke "
              "with 'test_readercoin-qt -platform cocoa' on mac, or else use a linux or windows build.");
        return;
    }
#endif
    ...
    connect(&m_app, &ReadercoinApplication::windowShown, this, &AppTests::guiTests);
...
//! Entry point for ReadercoinGUI tests.
void AppTests::guiTests(ReadercoinGUI* window)
{
    HandleCallback callback{"guiTests", *this};
    connect(window, &ReadercoinGUI::consoleShown, this, &AppTests::consoleTests);
