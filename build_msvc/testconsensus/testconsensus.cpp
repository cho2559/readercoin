...
// readercoin includes.
#include <..\src\script\readercoinconsensus.h>
...
int main()
{
    std::cout << "readercoinconsensus version: " << readercoinconsensus_version() << std::endl;
    ...
    auto op0Result = readercoinconsensus_verify_script_with_amount(pubKeyScript.data(), pubKeyScript.size(), amount, (const unsigned char*)&stream[0], stream.size(), 0, readercoinconsensus_SCRIPT_FLAGS_VERIFY_ALL, &err);
