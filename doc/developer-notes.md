...
- [Development guidelines](#development-guidelines)
    - [General Readercoin Core](#general-readercoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Readercoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Readercoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/readercoin/readercoin/blob/master/contrib/valgrind.supp))
which includes known Valgrind warnings in our dependencies that cannot be fixed
in-tree. Example use:

```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_readercoin
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_readercoin --log_level=test_suite
$ valgrind -v --leak-check=full src/readercoind -printtoconsole
```
...
# A coverage report will now be accessible at `./test_readercoin.coverage/index.html`.
```
...
To profile a running readercoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep readercoind` -- sleep 60
```
...
Readercoin Core can be compiled with various "sanitizers" enabled, which add
 ...
 * [Issue #12691: Enable -fsanitize flags in Travis](https://github.com/readercoin/readercoin/issues/12691)
...
- Use include guards to avoid the problem of double inclusion. The header file
  `foo/bar.h` should use the include guard identifier `READERCOIN_FOO_BAR_H`, e.g.

```c++
#ifndef READERCOIN_FOO_BAR_H
#define READERCOIN_FOO_BAR_H
...
#endif // READERCOIN_FOO_BAR_H
```
...
- src/libsecp256k1
  - Upstream at https://github.com/readercoin-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/readercoin-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/readercoin-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors it will cause problems with Readercoin's `select()` loop, because
...
compatibility between nodes. This happened in Readercoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Readercoin's
...
Commit [`bb81e173`](https://github.com/readercoin/readercoin/commit/bb81e173) is an example of a scripted-diff.
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Readercoin Core, the wallet was always
